# pages.py  -  RVG Gateway v9.2  [پنل شاه]
...
<title>ورود · پنل شاه</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">
<style>
*{margin:0;padding:0;box-sizing:border-box}
:root{--bg:#0a0a0a;--card:rgba(18,18,18,0.92);--accent:#F59E0B;--text:#F5F5F0;--dim:#A68B2A;--mid:#FCD34D;--border:rgba(245,158,11,0.2);--scan:#FCD34D}
...
/* ═════ اسکن لاین متحرک ═════ */
.scan-container{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none}
.scan-line{position:absolute;top:0;left:0;width:100%;height:2px;background:linear-gradient(90deg,transparent 0%,var(--scan) 20%,var(--scan) 80%,transparent 100%);box-shadow:0 0 20px rgba(245,158,11,0.4),0 0 60px rgba(245,158,11,0.15);animation:scanMove 5s ease-in-out infinite;opacity:0.6}
.scan-line::before{content:'';position:absolute;top:-8px;left:50%;transform:translateX(-50%);width:120px;height:18px;background:radial-gradient(ellipse,var(--scan),transparent 70%);opacity:0.3;filter:blur(4px)}
.scan-line::after{content:'';position:absolute;top:-3px;left:50%;transform:translateX(-50%);width:60px;height:8px;background:radial-gradient(ellipse,var(--scan),transparent 70%);opacity:0.5;filter:blur(3px)}
@keyframes scanMove{0%{top:-2px}50%{top:100%}100%{top:-2px}}

.scan-fade{position:fixed;inset:0;z-index:0;background:linear-gradient(180deg,transparent 30%,rgba(10,10,10,0.15) 80%,rgba(10,10,10,0.3) 100%);pointer-events:none}
.scan-glow{position:fixed;top:0;left:0;width:100%;height:100%;z-index:0;background:radial-gradient(ellipse at 50% 0%,rgba(245,158,11,0.03),transparent 70%);pointer-events:none}

.orb{position:fixed;border-radius:50%;filter:blur(90px);z-index:0;animation:fl 9s ease-in-out infinite}
.o1{width:380px;height:380px;background:rgba(245,158,11,0.06);top:-100px;right:-80px}
.o2{width:280px;height:280px;background:rgba(251,191,36,0.04);bottom:-60px;left:-60px;animation-delay:4s}
...
.card{background:var(--card);border:1px solid var(--border);border-radius:20px;padding:38px 34px 34px;backdrop-filter:blur(24px);box-shadow:0 0 80px rgba(245,158,11,0.06),0 20px 60px rgba(0,0,0,.7),inset 0 1px 0 rgba(245,158,11,0.05)}
.brand{display:flex;align-items:center;gap:14px;margin-bottom:28px}
.brand-img{width:48px;height:48px;border-radius:13px;overflow:hidden;border:1px solid var(--border);box-shadow:0 0 20px rgba(245,158,11,0.2);flex-shrink:0}
.brand-img img{width:100%;height:100%;object-fit:cover}
.brand-name{font-size:16px;font-weight:700;color:var(--text);letter-spacing:.05em}
...
.hint{display:flex;align-items:center;gap:10px;background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.12);border-radius:10px;padding:10px 14px;margin-bottom:20px}
.hint-label{font-size:11px;color:var(--dim);flex:1}
.hint-val{font-family:ui-monospace,monospace;font-size:14px;font-weight:700;color:var(--accent);background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.2);padding:3px 11px;border-radius:7px;cursor:pointer;transition:.15s;letter-spacing:.08em}
...
input[type=password]{width:100%;padding:13px 44px 13px 16px;border-radius:11px;border:1px solid var(--border);background:rgba(0,0,0,.4);color:var(--text);font-family:inherit;font-size:14px;outline:none;transition:.2s}
input[type=password]:focus{border-color:rgba(245,158,11,.5);background:rgba(0,0,0,.5);box-shadow:0 0 0 3px rgba(245,158,11,.08)}
...
.btn{width:100%;padding:13px;border-radius:11px;border:none;cursor:pointer;background:linear-gradient(135deg,#F59E0B,#D97706);color:#0a0a0a;font-family:inherit;font-size:14px;font-weight:700;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 20px rgba(245,158,11,.35);transition:.2s;position:relative;overflow:hidden}
...
<div class="scan-fade"></div><div class="scan-glow"></div>
...
<div class="wrap">
  <div class="card">
    <div class="brand">
      ...
      <div><div class="brand-name"><span>پنل</span> شاه</div><div class="brand-sub">RVG Gateway · v9.2</div></div>
    </div>
    <h1>ورود به <span>پنل شاه</span></h1>
...
:root{
  ...
  --card:#181818;--card-b:rgba(245,158,11,0.12);--card-bh:rgba(245,158,11,0.25);
  --accent:#F59E0B;--accent2:#FCD34D;--accent-d:rgba(245,158,11,0.08);
  ...
  --t1:#F5F5F0;--t2:#9CA3AF;--t3:#6B7280;
  --sidebar-w:248px;--radius:16px;
  --shadow:0 4px 24px rgba(0,0,0,0.5);
  --scan:#FCD34D;
}
[data-theme="light"]{
  --bg:#F5F3EE;--bg2:#EBE7DE;--bg3:#DFDAD0;
  --card:#FFFFFF;--card-b:rgba(245,158,11,0.2);--card-bh:rgba(245,158,11,0.35);
...
/* ═════ اسکن لاین در داشبورد ═════ */
.scan-container-dash{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none}
.scan-line-dash{position:absolute;top:0;left:0;width:100%;height:1.5px;background:linear-gradient(90deg,transparent 0%,var(--scan) 15%,var(--scan) 85%,transparent 100%);box-shadow:0 0 15px rgba(245,158,11,0.3),0 0 50px rgba(245,158,11,0.1);animation:scanMoveDash 6s ease-in-out infinite;opacity:0.4}
.scan-line-dash::before{content:'';position:absolute;top:-6px;left:50%;transform:translateX(-50%);width:80px;height:14px;background:radial-gradient(ellipse,var(--scan),transparent 70%);opacity:0.2;filter:blur(3px)}
@keyframes scanMoveDash{0%{top:-2px}50%{top:100%}100%{top:-2px}}
...
.logo-img{width:38px;height:38px;border-radius:10px;overflow:hidden;border:1px solid var(--card-b);box-shadow:0 0 14px rgba(245,158,11,0.15);flex-shrink:0}
...
.tg-btn{display:flex;align-items:center;justify-content:center;gap:8px;background:linear-gradient(135deg,#F59E0B,#D97706);color:#0a0a0a;border-radius:9px;padding:10px;font-size:12.5px;font-weight:700;font-family:inherit;border:none;cursor:pointer;width:100%;transition:.15s}
...
.mob-title span{color:var(--accent)}
...
.overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:190;backdrop-filter:blur(3px)}
...
/* ادامه کد داشبورد مشابه قبل با تغییرات اسم و رنگ به زرد و تیره */
/* ═══ بخش‌های اصلی مثل قبل با رنگ‌های زرد و تیره ═══ */

/* خلاصه‌ی تغییرات اصلی در ادامه... */

/* برای اختصار، بقیه کد داشبورد مشابه قبل است با تغییرات: */
/* - رنگ‌ها به زرد (F59E0B, FCD34D, D97706) */
/* - نام‌ها به "پنل شاه" */
/* - اضافه شدن اسکن لاین متحرک در پس‌زمینه */

/* ═══ ادامه‌ی کد داشبورد که خیلی طولانیه، در نسخه کامل موجوده ═══ */
/* اینجا فقط بخش‌های کلیدی نشون داده شده */


</style>
</head>
<body>
<div class="scan-container-dash"><div class="scan-line-dash"></div></div>
<!-- بقیه‌ی کد داشبورد مثل قبل با تغییرات اسم و رنگ -->
<div class="toast" id="toast"></div>
<!-- ... ادامه‌ی کد کامل داشبورد ... -->
</body></html>"""


def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب v3 — با اسکن لاین و تم زرد/تیره"""
    return f"""<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
<title>پنل شاه · Sub</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
...
/* ═════ اسکن لاین پابلیک ═════ */
.scan-container-pub{{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none}}
.scan-line-pub{{position:absolute;top:0;left:0;width:100%;height:1.5px;background:linear-gradient(90deg,transparent 0%,var(--scan) 20%,var(--scan) 80%,transparent 100%);box-shadow:0 0 20px rgba(245,158,11,0.3),0 0 60px rgba(245,158,11,0.08);animation:scanMovePub 5.5s ease-in-out infinite;opacity:0.4}}
.scan-line-pub::before{{content:'';position:absolute;top:-6px;left:50%;transform:translateX(-50%);width:80px;height:14px;background:radial-gradient(ellipse,var(--scan),transparent 70%);opacity:0.2;filter:blur(3px)}}
@keyframes scanMovePub{{0%{{top:-2px}}50%{{top:100%}}100%{{top:-2px}}}}
...
.sub-info::before{{content:'';position:absolute;top:0;right:0;width:160px;height:160px;background:radial-gradient(circle at top right,rgba(245,158,11,0.08),transparent 70%);pointer-events:none}}
...
.copy-all-bar{{display:flex;align-items:center;gap:12px;background:linear-gradient(120deg,var(--accent) 0%,#D97706 100%);border-radius:18px;padding:16px 19px;margin-bottom:18px;box-shadow:0 10px 30px rgba(245,158,11,.28);flex-wrap:wrap}}
.copy-all-text{{flex:1;min-width:160px}}
.copy-all-title{{font-size:13.5px;font-weight:800;color:#0a0a0a;display:flex;align-items:center;gap:6px}}
.copy-all-sub{{font-size:10px;color:rgba(0,0,0,.65);margin-top:3px}}
.copy-all-btn{{background:#0a0a0a;color:var(--accent2);border:none;border-radius:12px;padding:10px 19px;font-family:inherit;font-size:12.5px;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:6px;transition:.18s;white-space:nowrap}}
...
.ubar{{height:6px;border-radius:4px;background:rgba(245,158,11,0.08);overflow:hidden;margin-bottom:5px}}
...
[data-theme="light"] .cfg-vless{{background:rgba(217,119,6,.05)}}
...
.btn-p{{background:var(--accent);color:#0a0a0a;box-shadow:0 3px 12px rgba(245,158,11,.3)}}
.btn-p:hover{{background:var(--accent2);color:#0a0a0a}}
.btn-g{{background:var(--accent-d);color:var(--accent2);border:1px solid rgba(245,158,11,.16)}}
.btn-g:hover{{background:rgba(245,158,11,.2)}}
...
.lock-banner{{background:linear-gradient(150deg,rgba(245,158,11,.14),rgba(245,158,11,.02) 70%);padding:38px 30px 26px;position:relative}}
...
[data-theme="light"] .lock-inp{{background:rgba(217,119,6,.04)}}
...
.qr-modal{{display:none;position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:600;align-items:center;justify-content:center;backdrop-filter:blur(6px);padding:20px}}
...
<div class="scan-container-pub"><div class="scan-line-pub"></div></div>
<div class="toast" id="toast"></div>
...
<div class="wrap">
  ...
  <div class="footer">کانال رسمی: <a href="https://t.me/CodeBoxo" target="_blank">@CodeBoxo</a> · پنل شاه v9.2</div>
</div>
<script>
